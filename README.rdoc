...
Domain.find_by(domain_name: 'Windows').metrics.where(device_flag: 1)

Windowsドメインのメトリックでdevice_flagが1の行を複数行取得

Domain.find_by(domain_name: 'Windows').metrics.where(metric_name: 'filesystem')
...
http://ostrich:3000/device_inventory?device_name=filesystem&server_name=w2016
